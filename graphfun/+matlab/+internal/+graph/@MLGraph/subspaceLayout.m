...
if ncomp > 1
    ...
    xyz(:, [1 2]) = matlab.internal.graph.packLayouts(xyz(:, [1 2]),compcell,comp);
else
    xyz = layoutOneConnComp(G,dim,1:G.NodeCount,L,deg,outdim);
end
x = xyz(:,1);
y = xyz(:,2);
z = xyz(:,3);
...
% Compute graph Laplacian by ignoring self-loops.
nnodes = G.NodeCount;
A = adjacency(G);
degselfloops = 2*diag(A);
deg = degree(G) - degselfloops(:);
A(1:nnodes+1:end) = 0;
