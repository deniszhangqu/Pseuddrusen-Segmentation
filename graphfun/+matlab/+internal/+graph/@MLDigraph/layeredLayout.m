function [nodesCoord, edgesCoord, orderedLayers] = layeredLayout(g, sources, sinks, asgnLay)
...
[h, hlayers, isInnerNode, edge2nodes] = constructHelper(gdag, layers, g);
...
edgesCoord = extractEdges(g, h, hlayers, xcoord, ycoord, edge2nodes);
...
[~, layers] = acyclicShortestPaths(h, -ones(h.EdgeCount, 1), nn+2);
layers = layers(1:nn);
...
edge2nodesDAG = cell(1, gdag.EdgeCount);

nodeIter = gdag.NodeCount + 1;
edgeIter = gdag.EdgeCount + 1;

for e=1:gdag.EdgeCount
    tail = edges(e, 1);
    head = edges(e, 2);
...
isInnerNode = [false(1, gdag.NodeCount), true(1, nn-gdag.NodeCount)];

% Translate edge2nodesDAG (for gdag) to edge2nodes (for g)
edge2nodes = cell(1, g.EdgeCount);
...
function edgeCoords = extractEdges(g, h, hlayers, xcoord, ycoord, edge2nodes)
% Assume that first numnodes(g) nodes of h are identical to nodes of g.

nodeCoords = [xcoord, ycoord];
edgeCoords = cell(g.EdgeCount, 1);
...
% For self-loops
circle = selfLoopRadius*[1-cosd(0:10:360); sind(0:10:360)].';

% For two-cycles
cycDist = selfLoopRadius;
twoCycle = adjacency(g, 'transp');
twoCycle = twoCycle & twoCycle.';

edges = g.Edges;

for ii=1:g.EdgeCount
    ...
    if tail == head
        ...
        edgeC = bsxfun(@plus, rotcircle, nodeCoords(tail, :));
        
    else
        
        edgeC = nodeCoords(edge2nodes{ii}, :);
        
        if twoCycle(tail, head) || hlayers(tail) == hlayers(head)
            if abs(hlayers(tail) - hlayers(head)) > 1
                if hlayers(tail) > hlayers(head)
                    edgeC(2:end-1, 1) = edgeC(2:end-1, 1) + cycDist;
                else
                    edgeC(2:end-1, 1) = edgeC(2:end-1, 1) - cycDist;
                end
            else
