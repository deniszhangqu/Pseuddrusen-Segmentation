...
if ~isscalar(src)
    ...
    subset = src;
    if isempty(w)
        H = matlab.internal.graph.MLDigraph(adjacency(G.Underlying), 'transp');
    else
        M = adjacency(G.Underlying, 1:numedges(G));
...
[d, pred] = applyOneToAll(H, w, singleNode, subset, method, nodeNames);
...
function [d, pred] = applyOneToAll(H, w, src, target, methodStr, nodeNames)
...
elseif methodStr == "acyclic"
    [noCycles, d, pred] = acyclicShortestPaths(H, w, src);
    if ~noCycles
...
elseif pathFormat == "cell"
    ...
    tails = tails(pred>0);
    if reverseDirection
        edgeind = findedge(G, tails, heads);
        tree = digraph(tails, heads, G.EdgeProperties(edgeind, :), G.Nodes);
    else
        edgeind = findedge(G, heads, tails);
        tree = digraph(heads, tails, G.EdgeProperties(edgeind, :), G.Nodes);
    end
end
