...
[method, source, target, useNodeNames] = parseFlags(G, varargin{:});
...
elseif method == "positive"
    ...
    D = johnsonAllShortestPaths(G, w, source, target, useNodeNames);
end


function D = johnsonAllShortestPaths(G, w, source, target, useNodeNames)

n = numnodes(G);
M = adjacency(G.Underlying, 'transp');
M(n+1, n+1) = 0;
M(1:n, n+1) = 1;
...
D = dijkstraAllShortestPaths(G.Underlying, wmod, source, target)';

if ischar(target)
    D = bsxfun(@plus, D, minweight.');
else
    D = bsxfun(@plus, D, minweight(target).');
end
if ischar(source)
    D = bsxfun(@minus, D, minweight);
else
    D = bsxfun(@minus, D, minweight(source));
end

function [methodStr, source, target, useNodeNames] = parseFlags(G, varargin)
...
if setSRC
    source = s;
    useNodeNames = useNodeNames && ~isnumeric(varargin{1});
    varargin(1) = [];
end
