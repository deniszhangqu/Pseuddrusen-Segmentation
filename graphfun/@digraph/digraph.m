...
%   Copyright 2014-2016 The MathWorks, Inc.
    ...
    methods
        ...
        %   matrix with A(i,j) = 1 if (i,j) is an edge in digraph G.
        %   Otherwise, A(i,j) = 0.
        %
        %   Example:
        ...
        I = incidence(G);
        % Algorithms
        [path, d] = shortestpath(G, s, t, varargin);
        [tree, d] = shortestpathtree(G, s, varargin);
        ...
        H = transreduction(G);
        H = transclosure(G);
        [bins, bins2] = conncomp(G, varargin);
        C = condensation(G);
    ...
    methods (Hidden)
        ...
        function bctree(varargin)
            error(message('MATLAB:graphfun:digraph:OnlyGraphSupported'));
        end
        function biconncomp(varargin)
            error(message('MATLAB:graphfun:digraph:OnlyGraphSupported'));
        end
        function degree(varargin)
            error(message('MATLAB:graphfun:digraph:NoDegreeDir'));
        end
        function laplacian(varargin)
            error(message('MATLAB:graphfun:digraph:OnlyGraphSupported'));
        end
        function minspantree(varargin)
            error(message('MATLAB:graphfun:digraph:OnlyGraphSupported'));
        end
        function neighbors(varargin)
    ...
    methods (Static, Access = private)
        ...
        function ind = partialMatch(name, candidates)
            n = max(strlength(name), 1);
            ind = strncmpi(name, candidates, n);
        end
    end
end
