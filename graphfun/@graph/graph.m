...
%   Copyright 2014-2017 The MathWorks, Inc.
    ...
    methods
        ...
        %   A = ADJACENCY(G) returns a numnodes(G)-by-numnodes(G) sparse
        %   matrix with A(i,j) = 1 if (i,j) is an edge in graph G.
        %   Otherwise, A(i,j) = 0.
        %
        %   Example:
    ...
    methods (Hidden)
        ...
        function condensation(varargin)
           error(message('MATLAB:graphfun:graph:OnlyDigraphSupported'));
        end
        function flipedge(varargin)
           error(message('MATLAB:graphfun:graph:OnlyDigraphSupported'));
        end
        function indegree(varargin)
           error(message('MATLAB:graphfun:graph:NoInDegreeUndir'));
        end
        function isdag(varargin)
           error(message('MATLAB:graphfun:graph:OnlyDigraphSupported'));
        end
        function outdegree(varargin)
           error(message('MATLAB:graphfun:graph:NoOutDegreeUndir'));
        end
        function predecessors(varargin)
           error(message('MATLAB:graphfun:graph:NoPredecessorsUndir'));
        end
        function successors(varargin)
           error(message('MATLAB:graphfun:graph:NoSuccessorsUndir'));
        end
        function toposort(varargin)
           error(message('MATLAB:graphfun:graph:OnlyDigraphSupported'));
        end
        function transclosure(varargin)
           error(message('MATLAB:graphfun:graph:OnlyDigraphSupported'));
        end
        function transreduction(varargin)
           error(message('MATLAB:graphfun:graph:OnlyDigraphSupported'));
        end
