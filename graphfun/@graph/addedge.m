...
%   t is not present in the graph G, that node is added as well.  s and t
...
inputsAreStrings = graph.isvalidstring(s) && graph.isvalidstring(t);
...
nn = numnodes(H);
A = sparse(s, t, 1, nn, nn);
A = A + A';

% Self-loops have duplicate entries, divide by two
ind = find(diag(A) ~= 0);
ind = ind + nn*(ind-1);
A(ind) = A(ind)/2;
...
Hadjacency = adjacency(H.Underlying);

if nnz(A ~= 0 & Hadjacency ~= 0) ~= 0
    A = sparse(s, t, 1, nn, nn);
    [s, t] = find(A ~= 0 & Hadjacency ~= 0, 1);
    if inputsAreStrings
        s = H.NodeProperties.Name{s};
        t = H.NodeProperties.Name{t};
    end
