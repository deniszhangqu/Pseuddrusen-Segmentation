function t = search(isBFS, G, s, varargin)
...
if formatTable
    ...
    t = table(categorical(t(:, 1), 1:6, searchLabels), nodes, edges, ...
            'VariableNames', {'Event', 'Node', 'Edge'});
else
    if ischar(s) && hasNodeNames(G)
        t = G.Nodes.Name(t);
    end
...
if numel(varargin) > 0
    ...
    for ii=1:2:numel(varargin)
        ...
        if graph.partialMatch(name, "Restart")
